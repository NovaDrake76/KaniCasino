      ...
      <span className="relative flex h-12 w-12 items-end justify-center overflow-hidden">
        ...
        {(attention || hurry) && (
          <span className="absolute right-0 top-0 flex h-2.5 w-2.5">
            ...
            <span className={`relative h-2.5 w-2.5 ${hurry ? "bg-accent-amber" : "bg-accent-gold"}`} />
          </span>
        )}
      </span>
      {bubbleGift ? (
        ...
        <span className="flex flex-col leading-tight">
          ...
          {bubbleBonus ? (
            <span className={`mt-0.5 flex items-center gap-1 text-[11px] font-semibold tabular-nums ${hurry ? "text-accent-amber" : "text-ink-soft"}`}>
              <FiClock aria-hidden />
              {i18n.t("daisu.bonusBubble", { game: bubbleBonus.name, clock: bubbleBonus.clock })}
            </span>
          ) : (
