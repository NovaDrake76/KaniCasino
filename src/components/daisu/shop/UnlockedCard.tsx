...
interface Props {
  item: ShopItem;
  // a boost or a charm has nowhere to be shown, so its card only closes
  onShowMe?: () => void;
  onLater: () => void;
}
...
const UnlockedCard = ({ item, onShowMe, onLater }: Props) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-end justify-center text-white md:items-center" style={{ background: "rgba(9, 7, 20, 0.78)" }}>
      <div
        role="dialog"
        aria-label={`${t("unlocked")} · ${words.name}`}
      ...
      >
        <span className="flex h-[92px] w-[92px] items-center justify-center bg-surface-nav md:h-[110px] md:w-[110px]" style={{ boxShadow: "0 0 44px 8px rgba(255, 204, 0, 0.25)" }}>
          <ShopArt item={item.key} game={item.game} size={64} />
        </span>
        <span className="text-[11px] font-bold uppercase tracking-wider text-accent-gold">{t("unlocked")}</span>
        <span className="text-2xl font-extrabold leading-tight md:text-[28px]">{words.name}</span>
        <div className="flex items-start gap-3 bg-surface-nav p-3 text-left">
          ...
          <p className="m-0 text-[13px] leading-normal text-ink-soft"><TypedText text={words.opened} /></p>
        </div>
